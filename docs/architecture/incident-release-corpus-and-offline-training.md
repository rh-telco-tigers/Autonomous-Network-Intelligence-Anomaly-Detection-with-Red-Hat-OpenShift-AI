# IMS Incident Release Corpus and Offline Training Contract
...
This document is a cross-phase release and offline-training specification.

Primary phase alignment:

- Phase 1: Data Generation, because the release starts from persisted feature windows, incident history, approvals, audit events, and RCA artifacts already generated by the live platform
- Phase 2: Feature Store, because the released schema must remain compatible with future structured feature projections
- Phase 3: Model Training (KFP), because deterministic split generation and training eligibility are part of the offline-training contract
- Phase 4: Model Registry, because release identity and source snapshot lineage must flow into downstream model metadata

This document is not the primary specification for live serving, real-time RCA orchestration, or remediation execution. Those are covered in [feature-store-training-path](./feature-store-training-path.md), [engineering-spec](./engineering-spec.md), and [rca-remediation](./rca-remediation.md).

### 1.3 Document Role

Keep this document as the detailed release-data contract.

Use this file when you need:

- source-of-truth rules for release construction
- dataset layers, manifest requirements, and public artifact definitions
- linkage, eligibility, taxonomy, privacy, and split-policy rules
- release pipeline stages, quality gates, and acceptance criteria

Use the phase overview files for quick navigation. They do not replace this contract-level detail.
